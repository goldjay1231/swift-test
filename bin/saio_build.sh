...
SWIFT_HOME=${1:-.}
SWIFT_PATH=${SWIFT_HOME}/VENV/bin/
...
# = Init =
# Reset
cmd "umount -l /mnt/sdb1 || true"
cmd rm -fr /mnt/sdb1 /srv/*/node

# = Main =
wr "[Create Loop devices for storing objects]"
cmd sudo dd if=/dev/zero of=/srv/swift-disk bs=1024 count=0 seek=1000000
cmd sudo mkfs.xfs -f -i size=1024 /srv/swift-disk
#cat >> /etc/fstab <<EOF
#/srv/swift-disk /mnt/sdb1 xfs loop,noatime,nodiratime,nobarrier,logbufs=8 0 0
#EOF
cmd sudo mkdir -p /mnt/sdb1
cmd sudo mount -t xfs -o loop,noatime,nodiratime,nobarrier,logbufs=8 /srv/swift-disk /mnt/sdb1

cmd sudo mkdir -p /mnt/sdb1/1 /mnt/sdb1/2 /mnt/sdb1/3 /mnt/sdb1/4
for x in {1..4}; do cmd ln -s /mnt/sdb1/$x /srv/$x; done
cmd sudo mkdir -p /srv/1/node/sdb1 /srv/2/node/sdb2 /srv/3/node/sdb3 /srv/4/node/sdb4 /var/run/swift
...
cmd sed 's/RSYNC_ENABLE=false/RSYNC_ENABLE=true/g' /etc/default/rsync -i
...
wr "[Generate Configuration Files]"

cmd mkdir -p /etc/swift /etc/swift/account-server /etc/swift/container-server /etc/swift/object-server
